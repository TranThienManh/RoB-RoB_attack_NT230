...
def predict_new_data(df, model_filepaths, scaler_filepath, true_label):
    ...
    y_true = [true_label] * len(X_scaled)
    ...
    for model_filepath in model_filepaths:
        ...
        if cm.shape == (2, 2):
            ...
            tn = fp = tp = 0
            fn = len(y_pred) if true_label == 1 else 0
...
#them vao
def save_prediction_results(df, model_filepath, scaler_filepath, output_file='prediction_results.csv'):
    with open(scaler_filepath, "rb") as file:
        scaler = pickle.load(file)
...
def main():
    ...
    if args.test:
        ...
        #them vao
        #save_malicious_files(test_df, model_filepaths[0], args.scaler, args.true_label, 'malicious_files.csv')
        save_prediction_results(test_df, model_filepaths[-1], args.scaler, 'prediction_results.csv')
